...
# Per-workspace (all five workspaces have their own vitest.config.ts)
...
- **The built app dies at launch with `SyntaxError: Named export 'x' not found`** — a named import from a CommonJS package listed in esbuild's `external`. The main bundle is ESM, so Node resolves those imports through `cjs-module-lexer`, which statically scans `module.exports` and cannot see through nested object literals or spreads. `ssh2`'s `utils` is the known case (`Client` is detected, `utils` is not). Import the default and destructure. `tsc` and the unit tests both pass regardless — only launching the built app catches it. Check with `node --input-type=module -e "import('pkg').then(m=>console.log(Object.keys(m)))"`.
...
- **Stale TUI rows in local shells after a pane/window gets wider** — ConPTY reflows its buffer with conhost's algorithm on width changes and then emits only the rows it believes changed; xterm.js reflows differently, so rows ConPTY skips survive as ghosts (claude's full-width, right-aligned UI is the loudest victim; Windows Terminal is immune because it shares conhost's reflow). No xterm option fixes it — `windowsPty` was tested and doesn't help. `conptyResyncWiggle.ts` resyncs by narrowing 2 columns for 150ms and restoring once a widening burst settles; it looks decorative but is load-bearing.
...
- **A dialog, popup or menu renders *under* the terminal** — native ghostty surfaces are child HWNDs and always paint above the web contents, whatever the z-index says. Any new fixed-position or portaled overlay must call `useOverlayGuard(open)` (`features/terminal/nativeOverlayGuard.ts`), which hides every non-exempt surface while it is up. The same HWND swallows mouse events, so an overlay that isn't registered is not just invisible — its buttons are unclickable.
- **Occluding a terminal means hiding the surface, never lowering it** — the only supported way to cover a native surface is `useOverlayGuard` (hide/show). Pushing the surface to `HWND_BOTTOM` will silently not work: the ghostty host re-raises its leaf to `HWND_TOP` on create, on every bounds sync, on `setVisible(true)`, and from its own `WM_WINDOWPOSCHANGED` handler, so the demotion is undone within milliseconds. That re-assert is load-bearing — it is what keeps Chromium's `Chrome_RenderWidgetHostHWND` and `Intermediate D3D Window` siblings from painting over the terminal — and `surfaceZOrderResync.ts` deliberately re-triggers it (immediately plus a few bounded follow-ups) on the events that reorder those siblings without notifying our child window: a GPU-process restart, a renderer crash, sleep/wake, and display hot-plug or DPI change. A re-sync cannot un-hide a guarded surface — the host's `opSetBounds` passes `SWP_NOACTIVATE` with no `SWP_SHOWWINDOW`.
...
- **Ghostty terminal panes blank in dev** — a dev run (`pnpm --filter @hypershell/desktop dev`) needs `GHOSTTY_HOST_PATH` pointing at a built `ghostty-host.exe` (e.g. the ghostty port repo's `zig-out\bin\ghostty-host.exe`); without it every terminal pane is created against a nonexistent exe. The pane now shows the resolution error with a Retry button instead of staying silently blank. Packaged builds bundle the host and need nothing.
