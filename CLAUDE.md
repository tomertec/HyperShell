...
- **`pnpm --filter @hypershell/session-core test` fails with "No projects were found"** — that package, plus `shared` and `db`, has a `test` script but no `vitest.config.ts`, so vitest resolves the root config whose `projects: ["apps/*", "packages/*"]` globs match nothing from inside the package. Only `ui` and `desktop` have their own config. Run the others from the repo root: `npx vitest run --project @hypershell/session-core`.
